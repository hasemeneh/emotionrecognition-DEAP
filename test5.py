...
activeThread = 0
count = 0
selectedChannel = [0,16,2,19,10,28,7,25]
populatedData = np.zeros(36)
def populatePart(n):
	data = np.zeros(36)
	label = np.zeros(4)
	arrayX = np.array
	...
	for y in xrange(0,40):
		...
		bandPassed = None
		# stfted = stftX(temporalData,False)
		# temporalData = np.abs(temporalData)

		temporalData = temporalData.T
		temporalData = temporalData[3840:]
		...
		# 	tempPCA = vstackX((tempPCA,pca.fit(temporalData[(x*128):((x+1)*128)]).singular_values_))
		# tempPCA = tempPCA[1:]
		temporalData = hstackX((tempPCA,tobeprocessed))
		tempPCA = None
		tobeprocessed = None
		# temporalData = appendX(tempPCA,tobeprocessed)
		count = count+1
...
if __name__ == '__main__':
	...
	np.savetxt('data-bandpassed-theta-alpha-beta-gamma-pca.csv', populatedData, delimiter=',')
