...
def populatePart(n):
	...
	for y in xrange(0,40):
		...
		# temporalData = vstackX((temporalData,bandPassed[2].T))
		bandPassed = None
		# temporalData = fftX(temporalData,False)
		waveletedA , waveletedD = dwtX(temporalData,False)
		temporalData = waveletedA
		waveletedA = None
		temporalData = vstackX((temporalData,waveletedD))
		waveletedD = None
		...
		for x in xrange(0,21):
			tempPCA = vstackX((tempPCA,pca.fit(temporalData[(x*192):((x+1)*192)]).singular_values_))
		tempPCA = tempPCA[1:]
...
if __name__ == '__main__':
	...
	np.savetxt('data-bandpassed-dwt-pca.csv', populatedData, delimiter=',')
